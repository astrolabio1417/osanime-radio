...
import { OsAnimeRadio } from '../utils/radio'
...
const radio = new OsAnimeRadio(shuffle(songs))
radio.play()

async function updateRadio() {
  const osanimePlaylist: InterfaceOsanimeItem[] = await getAnimeSongs(100)
  const radioPlaylist: InterfaceRadioPlaylistItem[] = osanimePlaylist?.map((item) => ({
    id: uuidv4(),
    title: item?.title,
    file: item?.url,
    image: item?.image,
...
streamRoute.get('/', (req, res) => {
  ...
  const eventName = 'stream'
  res.setHeader('content-type', 'audio/mpeg')
  console.log(ipAddress, ' has joined the stream')

  function streamListener(chunk: any) {
    res.write(chunk)
  }
  radio.eventEmitter.on(eventName, streamListener)

  const responseFinishEventNames = ['close', 'end', 'error']
  responseFinishEventNames.map((name) =>
    req.on(name, () => {
      radio.eventEmitter.removeListener(eventName, streamListener)
      console.log(ipAddress, ' has been disconnected!')
    }),
  )
})
...
streamRoute.get('/:id/download', async (req, res) => {
  ...
  res.setHeader('content-type', 'audio/mpeg')
  res.setHeader('content-disposition', contentDisposition(`${result?.title}.mp3`))
