...
export class OsAnime {
  ...
  async list(page = '1', sort = 'newest') {
    ...
    const ostPageUrl = `${this.url}/page-lists/1/Ost-Anime/${sort}/${page || 1}`
  ...
  async download(url: string, range = 0) {
...
async function getAnimeSongs(limit = 10) {
  ...
  while (totalPerMultiFetch) {
    console.log(`Page: ${page}-${page + limitFetch} | Total: ${songList.length}`)
...
export { ostDownloader, getAnimeSongs }
